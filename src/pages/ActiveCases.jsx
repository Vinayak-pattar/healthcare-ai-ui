...
export default function ActiveCases() {
  ...
  const activePatients = patients.filter((patient) => patient.admitted === "Yes");
    ...
    <div className="space-y-10">
      <div className="flex flex-col gap-4 rounded-[32px] bg-white p-8 shadow-sm ring-1 ring-slate-200 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm text-slate-500">
            Monitor admitted patients and review their latest clinical updates. Organized by severity and case type.
          </p>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[2fr_1fr]">
        ...
        {selectedPatient ? (
                ...
                <div className="space-y-3">
                  {selectedPatient.reports.length > 0 ? (
                    selectedPatient.reports.map((report, index) => (
                      <button
                        key={index}
                        onClick={() => navigate(`/reports/${selectedPatient.id}`)}
          ...
          <div className="rounded-[32px] bg-white p-8 shadow-sm ring-1 ring-slate-200">
            <p className="text-slate-500">Select a patient card to preview case details and reports.</p>
          </div>
        )}
      </div>
    </div>
